(*
type zombie_properties = { hp : int; damage : int; distance : int; speed : int ; frame : int; row : int }
...
(* speed should be just constant because the speed of the pea is the same for all plants
   (at least for now) *)
type pea_properties = { damage : int; distance : int; speed : int; speedChange : int; row : int}
type lawnmower_properties = { damage : int; speed : int; distance : int; row : int }
